...
/**
 * The S3 related functionality of the plugin.
 *
...
class Auto_Backup_s3_API {
	...
	public function __construct() {
		...
		if(isset($data['s3'])){
			$this->bucket = $data['s3']['bucket_name'];
			$this->credentials = [
				'version'     => 'latest',
				'region'      => 'us-east-1',
				'credentials' => [
					'key'    => $data['s3']['access_key'],
					'secret' => $data['s3']['access_secret'],
				],
			];
		}
	...
	public function get_buckets() {
		...
		try {
			...
			echo esc_html__('Error: ') . esc_html($e->getAwsErrorMessage());
		}
	...
	public function create_buckets($bkt_name, $access_key, $secret_key) {
		...
		try {
			...
			return esc_html__('Error: ') . esc_html($e->getAwsErrorMessage());
		}
	...
	public function get_object($keyname, $serverlocation) {
		...
		$response = wp_remote_get("https://$bkt.s3.amazonaws.com/$keyname");
